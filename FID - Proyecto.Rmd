...
2.  Aprendizaje no supervisado: Agrupación de equipos en base a sus puntuaciones de defensa, centro, ataque y portería.
...
En nuestro dataset tenemos 25 columnas con 23.921 filas, con información sobre partidos desde 1993 a 2022. Estos partidos contienen los equipos que lo jugaron, dónde lo jugaron, cuándo lo jugaron, qué resultado obtuvieron, el torneo del partido y información relativa sobre el rendimiento en portería, defensa, mediocentro y ataque de ambos equipos.

Primero debemos decidir si para nuestro caso de estudio necesitaremos todos los ejemplos o podemos deshacernos de algunos para agilizar la computación.
...
-   **Amistosos:** En estos partidos existe un equipo local y un equipo visitante, pues se juegan en la ciudad del equipo local. Esto puede provocar que el resultado se vea afectado por factores externos (mayor cantidad de aficionados locales en el campo), al contrario que en partidos oficiales.
-   **Oficiales:** Estos partidos se juegan en terreno neutral, por lo que no hay equipo local ni visitante, de manera que factores externos como la cantidad de aficionados o el terreno de juego no influyen de manera significativa en el resultado. Esta es la situación que se ajusta mejor a las necesidades del algoritmo que se pretende obtener, pues se quieren predecir los resultados de los partidos del mundial de 2022, que son todos de carácter oficial.
...
supervised_friendly_partition <- trainControl(
                           method = "repeatedcv",
                           number = 3,
                           repeats = 3)

suppressWarnings(supervised_friendly_random_forest <- train(home_team_result ~.,
            data = supervised_friendly_training_set_encoded,
            method = "rf",
...
Podemos ver que ahora, la ciudad (atributo "city") ha aumentado su importancia considerablemente, aproximadamente unos 20 puntos. Esto se debe a que en los amistosos que se juegan, el equipo local tiene el apoyo de la afición y conoce el terreno de juego. Sabiendo que en el mundial no existe club local ni visitante, vamos a eliminar estas colunmas y volver a realizar este análisis.
...
suppressWarnings(supervised_random_forest <- train(home_team_result ~.,
            data = supervised_training_set_encoded,
            method = "rf",
            trControl = supervised_fit_control))
...
Ahora, con el dataset preprocesado, vamos a empezar a aplicar distintos algoritmos de predicción para obtener el que mejor se comporte. Para ello, vamos a aplicar primero NaiveBayes con los datos sin hacer LabelEncoding, y luego probaremos KNN y SVC con LabelEncoding.
...
Cabe destacar, que en este caso, hemos realizado unas modificaciones al set en Python, con los scripts `/scripts/cleaner.py` y `/scripts/scorer.py`. Como resumen del funcionamiento de estos scripts, toman el dataset original "international_matches", eliminamos los amistosos para deshacernos de la correlación de cuidad-torneo-pais y calculamos la media de puntos en portería, defensa, medio y ataque para cada país. Esta información la volcamos en el dataset "kmedias_input".

```{r}
teams_data <- read_csv("datasets/kmedias_input.csv")
teams_data_reversed <- as.data.frame(apply(teams_data, 2, rev))
teams_data_reversed
```

**Imputación de valores**

Sin embargo, tenemos valores faltantes dentro del dataset, como se puede ver en el caso de Qatar (se ha mostrado el dataset invertido para evidenciar esto). Para ello, usaremos la imputación de valores faltantes a través de la función preProcess de Caret con el método medianImpute.
...
teams_data_reversed <- as.data.frame(apply(teams_data, 2, rev))[-c(1)]
teams_data_reversed
```

Con medianImpute hemos obtenido unos valores que se ajustan a la realidad, ya que Qatar esta rankeada con valoraciones similares a equipos que sabemos que tienen el mismo nivel, como Ecuador o Ghana.

**Entrenamiento de múltiples modelos no supervisados - Clustering**

Ahora que ya tenemos todos los valores, vamos a entrenar distintos modelos de clustering para agrupar los equipos en función de sus puntuaciones de ataque, defensa, centro del campo y portería.
...
Cmeans_sqeuclidean_clustering <- fanny(teams_data, diss=FALSE, k=3, 
                                 metric="SqEuclidean", stand=FALSE)
```

Hemos obtenido 3 clusteres de equipos en cada uno de los modelos. Ahora vamos a evaluar los modelos obtenidos contra el ranking FIFA para seleccionar el mejor.

**Evaluación de los modelos**

En primer lugar, vamos a crear una agrupación de referencia basandonos en el ranking FIFA:

```{r}
team_cluster <- teams_data$team
team_ranking_ordered <- teams_data$team[order(teams_data$fifa_rank)]

mejores <- team_ranking_ordered[1:10]
medianos <- team_ranking_ordered[11:21]
peores <- team_ranking_ordered[22:32]

for (i in 1:length(team_cluster)) {
  if (team_cluster[i] %in% mejores) {
    team_cluster[i] <- 1
  } else if (team_cluster[i] %in% medianos) {
    team_cluster[i] <- 2
  } else {
    team_cluster[i] <- 3
  }
...
team_cluster <- as.data.frame(team_cluster)
team_cluster$team <- teams_data$team
team_cluster
```

Utilizaremos las métricas ARI (Adjusted Rand Index) y AMI (Adjusted Mutual Information), que determinan la disimilaridad del clustering con respecto a otro clustering, además de la silueta, medida que combina la distancia intraclúster e interclúster en su cálculo.

-   **ARI**: El índice ajustado de Rand (ARI) es una medida de la similitud entre dos agrupamientos que considera todos los pares de muestras y cuenta el número de ellas que están asignadas al mismo grupo en ambos agrupamientos. Tiene un rango de -1 a 1, siendo un valor de 1 indicativo de que los dos agrupamientos son exactamente iguales y un valor de -1 indicativo de que son exactamente opuestos. El ARI no es sensible al número de grupos, pero puede ser sensible al tamaño de los grupos.

-   **AMI**: La información mutua ajustada (AMI) es una medida de la similitud entre dos agrupamientos que se basa en la información mutua entre ellos. Tiene un rango de -1 a 1, siendo un valor de 1 indicativo de que los dos agrupamientos son exactamente iguales y un valor de -1 indicativo de que son exactamente opuestos. La AMI es más robusta que el ARI, pero puede ser sensible al número de grupos y puede no ser adecuada para conjuntos de datos muy pequeños.

-   **Silhouette:** La silueta es una medida que calcula la "cohesión" y la "separación" de una muestra dentro de un grupo y entre grupos. Tiene un rango de -1 a 1, siendo un valor de 1 indicativo de que la muestra está bien separada de otros grupos y un valor de -1 indicativo de que la muestra está cerca del límite entre dos grupos. La silueta no es sensible al número de grupos, pero puede no ser fiable cuando los grupos tienen tamaños o densidades muy diferentes.

Como medida de confianza para el cálculo del ARI y del AMI, utilizaremos el ranking FIFA real a modo de referencia para evaluar la agrupación obtenida por cada uno de nuestros modelos.
...
cmeans_sq_ari <- ARI(Cmeans_sqeuclidean_clustering$cluster, rank)
...
cmeans_sq_ami <- AMI(Cmeans_sqeuclidean_clustering$cluster, rank)
...
suppressWarnings({
  ...
  cmeans_sq_silhouette <- mean(as.data.frame(
            silhouette(Cmeans_sqeuclidean_clustering))$sil_width)
})

cat("Comparativa de ARIs")
cat(paste("\nKmeans ARI =", kmeans_ari))
cat(paste("\nCmeans ARI con distancia euclidea =", cmeans_e_ari))
cat(paste("\nCmeans ARI con distancia euclidea cuadrática =", cmeans_sq_ari))


cat("\n\nComparativa de AMIs")
cat(paste("\nKmeans AMI =", kmeans_ami))
cat(paste("\nCmeans AMI con distancia euclidea =", cmeans_e_ami))
cat(paste("\nCmeans AMI con distancia euclidea cuadrática =", cmeans_sq_ami))

cat("\n\nComparativa de Silhouette")
cat(paste("\nKmeans Silhouette mean =", kmeans_silhouette))
cat(paste("\nCmeans Silhouette mean con distancia euclidea =", cmeans_e_silhouette))
cat(paste("\nCmeans Silhouette mean  con distancia euclidea cuadrática =", cmeans_sq_silhouette))

```

**Selección del mejor modelo**

A la vista de los resultados arrojados por el cálculo de las métricas anteriores, y teniendo en cuenta que estamos ante un conjunto de datos de tamaño reducido, es más apropiado utilizar la silueta para decidir cuál de los modelos es mejor. Así, a pesar de ser el modelo Cmeans con distancia euclídea aquel que obtiene más puntuación en ARI y AMI, el modelo óptimo para este caso sería Cmeans con distancia euclídea cuadrática, ya que arroja resultados significativamente superiores en la silueta, lo cual es indicativo de que los grupos hallados tienen una alta cohesión y separación.

```{r}
unsupervised_final_model <- Cmeans_sqeuclidean_clustering
```

**Visualización de los clústeres del modelo seleccionado**

La siguiente función auxiliar muestra las gráficas de ataque, defensa, medio campo y portería obtenidas a partir del algoritmo de clustering cuyo resultado se especifique en el primer argumento.

```{r}
show_team_stats_plot <- function(cluster_df, cluster_col) {
  teams_data_copy <- teams_data
  
  result <- cluster_df[[cluster_col]]
    
  clusters <- vector("list", length = length(unique(result)))
  colormap <- colorRampPalette(c("red", "yellow", "green"))(length(unique(result)))

  teams_data_copy$color <- "#000000"

  for (val in unique(result)) {
    cluster <- which(result == val)
    clusters[[val]] <- c(teams_data_copy[cluster,])
    for (team in clusters[[val]]$team) {
      teams_data_copy[teams_data_copy$team == team, ]$color <- colormap[[val]]
    }
  }

  plot1 <- ggplot(teams_data_copy, aes(x = fifa_rank, y = goalkeeper_score, color = color)) + geom_point(show.legend = FALSE) + geom_text_repel(aes(label = team), show.legend = FALSE) + ggtitle("Goalkeeper Score")

  plot2 <- ggplot(teams_data_copy, aes(x = fifa_rank, y = mean_defense_score, color = color )) + geom_point(show.legend = FALSE) + geom_text_repel(aes(label = team), show.legend = FALSE) + ggtitle("Mean Defense Score")

  plot3<- ggplot(teams_data_copy, aes(x = fifa_rank, y = mean_offense_score, color = color)) + geom_point(show.legend = FALSE) + geom_text_repel(aes(label = team), show.legend = FALSE) + ggtitle("Mean Offense Score")

  plot4 <- ggplot(teams_data_copy, aes(x = fifa_rank, y = mean_midfield_score, color = color)) + geom_point(show.legend = FALSE) + geom_text_repel(aes(label = team), show.legend = FALSE) + ggtitle("Mean Midfield Score")
  
  suppressWarnings(gridExtra::grid.arrange(plot1, plot2, plot3, plot4))
}
```

A continuación, se presenta el agrupamiento realizado por el modelo de Cmeans con distancia euclídea cuadrática:

```{r}

# Cmeans euclidean cluster visualization
fviz_cluster(unsupervised_final_model, repel=TRUE, ellipse.type = "norm", pallete="jco") + theme_bw() + labs(title = "Squared Euclidean Cmeans fuzzy cluster plot")

show_team_stats_plot(unsupervised_final_model, "clustering")
```
...
-   Clúster 2: Selecciones medianas como México, España, Uruguay, Portugal o Croacia están agrupadas en un mismo clúster. Tiene sentido de nuevo, ya que estas selecciones suelen ser terceras de grupo o pasar a eliminatorias frecuentemente, aunque siendo eliminadas en fases relativamente tempranas (España en cuartos y octavos en 2018 y 2022, Portugal en cuartos en 2022...), pero son claramente mejores que las selecciones del cluster 1.

-   Clúster 3: las mejores selecciones. Aquí tenemos a Argentina (campeona 2022), Francia (Campeona 2018 y Subcampeona 2022), Brasil, Inglaterra... Las siempre favoritas.

Podemos ver que en los 4 casos planteados, (portería, defensa, mediocentro, ataque), tenemos los mismos clusteres con los mismos equipos. La distribución es muy parecida, aunque no siempre igual. Por ejemplo, España destaca en el centro del campo pero carece en ofensiva, al contrario que Brasil y Argentinca, que denotan un mejor ataque que defensa. De nuevo, tiene sentido ya que estas selecciones son conocidas por la gran cantidad de atacantes élite que ha mantenido.
...
10. Clustering con KMedias, Cmedias y evaluación con distintas métricas

11. Visualización del dataset, de atributos, de modelos, comparativas y clústeres

Es crucial trabajar bien el dataset y estructurar los datos de manera adecuada antes de pasar a implementar algoritmos de aprendizaje. Puede no existir un \"mejor\" modelo. Dependiendo de en qué casos, puede interesarnos más un clasificador que otro, incluso cuando en la evaluación haya obtenido peor puntuación.

Hemos podido conocer más a fondo el mundo del científico e ingeniero de datos, los procesos y tareas de un proyecto de inteligencia artificial y big data, aprender más sobre R y su utilidad para este sector y obtener varias conclusiones interesantes al respecto.
